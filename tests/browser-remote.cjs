...
const g=E.createGame({date:'2026-09-08',opponent:'Remote Hawks',category:'official',minutes:10,overtimeMinutes:5},roster);T.configure(g,roster.map(p=>p.id),roster.map(p=>p.id));E.start(g,1000);E.recordEvent(g,'2PT_MADE',roster[0],'HOME',1100);E.pause(g,61000);g.finished=true;
...
(async()=>{
 ...
 await send('Page.navigate',{url:base+'/index.html'});await until("document.querySelector('#period-select')?.disabled===false");await evaluate("document.querySelector('#period-select').value='career';document.querySelector('#period-select').dispatchEvent(new Event('change'))");await until("document.querySelector('#game-count')?.textContent==='1'");
 ...
 await evaluate('window.__rejectUpload=false');await click('#upload-complete');await value('#upload-pin','12345678');await click('#upload-submit');await until("document.querySelector('#upload-status').textContent.includes('Connection interrupted')");
 assert.deepEqual(await evaluate('window.__uploads[0].game_details'),{category:'official',duration_ms:60000,stats_complete:true});
 assert.equal(await evaluate("document.querySelector('#upload-complete').disabled"),true);
